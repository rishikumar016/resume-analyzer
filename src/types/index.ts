import { z } from "zod";
import type { ResumeSchema as JsonResume } from "@supastuff/json-resume-types";

// ── Re-export official JSON Resume type ──
...
// New analysis schema with JSON Resume and section-specific suggestions
export const jsonResumeAnalysisSchema = z.object({
  resume: z.record(z.string(), z.any()), // JsonResume object (flexible structure)
  overallScore: z.number().min(0).max(100),
  scores: z.object({
    relevance: z.number().min(0).max(100),
    experience: z.number().min(0).max(100),
...
export type JsonResumeAnalysis = z.infer<typeof jsonResumeAnalysisSchema>;
